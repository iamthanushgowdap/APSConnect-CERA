...
// Styled components and styled JSX-like CSS
...
export default function NotFoundPage() {
  useEffect(() => {
    const update = (e: any) => {
      if (e.acceleration && e.acceleration.x !== null) {
        document.documentElement.style.setProperty('--X', String(e.acceleration.x));
        document.documentElement.style.setProperty('--Y', String(e.acceleration.y));
      } else {
        document.documentElement.style.setProperty('--X', String((e.pageX || 0) / window.innerWidth - 0.5));
        document.documentElement.style.setProperty('--Y', String((e.pageY || 0) / window.innerHeight - 0.5));
      }
    };
  ...
  return (
    <main className="bg-background text-foreground flex items-center justify-center min-h-screen font-mono" id="app">
      <div style={StyledContainer}>
        <a href="/" style={StyledMessage as any} className="return-link">
          Return to happiness
        </a>
        <BearLogo className="bear-logo bear-logo--tears" />
        <GlitchyText className="four-oh-four__code" text="404" style={StyledCode as any} />
        <GlitchyText className="four-oh-four__code-message" text="Not found" style={StyledCodeMessage as any} />
      </div>
    </main>
  );
}
...
function GlitchyText({ text, className, style }: { text: string; className?: string; style?: React.CSSProperties }) {
  ...
  return (
    <h1 className={`glitchy-text ${className || ''}`} style={style}>
      <span className="glitchy-text__char--readable">{text}</span>
      {text.split('').map((char, idx) => {
        // Only generate random styles on client side to prevent hydration mismatch
        const charStyle = isClient ? {
          '--count': Math.random() * 5 + 1,
          fontSize: `${Math.random() * 30 + 20}px`,
          ...Array.from({ length: 10 }, (_, i) => [`--char-${i}`, `"${GLITCH_CHARS[Math.floor(Math.random() * GLITCH_CHARS.length)]}"`]).reduce((acc, [key, value]) => ({ ...acc, [key]: value }), {})
        } : {};
...
// BearLogo Component
function BearLogo({ className }: { className?: string }) {
  return (
    <svg
