...
import { uploadFile, createAssignment, updateAssignment } from '@/lib/supabase-utils';
...
export function AssignmentForm({ 
  ...
  const onSubmit = async (data: AssignmentFormValues) => {
    ...
    try {
      const assignmentId = initialData?.id || crypto.randomUUID();
      ...
      const createdAssignment = initialData?.id 
        ? await updateAssignment(initialData.id, {
            ...
            attachments: data.attachments ? uploadedAttachments : (initialData.attachments || []),
      ...
      toast({ title: "Error", description: `Failed to create assignment: ${errorMessage}`, variant: "destructive" });
    } finally {
      setIsLoading(false);
    }
