...
const Shuffle = ({
  ...
  useGSAP(
    () => {
      ...
      const teardown = () => {
        if (tlRef.current) {
          (tlRef.current as any).kill();
          tlRef.current = null;
        }
        if (wrappersRef.current.length) {
          wrappersRef.current.forEach(wrap => {
            const inner = (wrap as any).firstElementChild;
            const orig = inner?.querySelector('[data-orig="1"]');
            if (orig && (wrap as any).parentNode) (wrap as any).parentNode.replaceChild(orig, wrap);
          });
          wrappersRef.current = [];
        }
        try {
          if (splitRef.current) (splitRef.current as any).revert();
        } catch {
          /* noop */
        }
